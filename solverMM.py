
import numpy as np
import matplotlib.pyplot as plt
import json
import math
import copy


def calcDispFromPYCurve(p, pyCurve):
    coeff = pyCurve["coeff"]
    pLim = pyCurve["pLim"]
    if len(coeff) == 1:
        if p < pLim[0]:
            p = pLim[0]
        elif pLim[1] <= p:
            p = pLim[1]
        x = (p - coeff[0][1]) / coeff[0][0]
    else:
        if p < pLim[0]:
            index = 0
            p = pLim[0]
        elif pLim[0] <= p and pLim[1]:
            index = 0
        elif pLim[1] <= p and pLim[2]:
            index = 1
        elif pLim[2] <= p and pLim[3]:
            index = 2
        else:
            index = 2
            p = pLim[3]
        x = (p - coeff[index][1]) / coeff[index][0]
    return x


def calcForceFromPYCurve(x, pyCurve):
    coeff = pyCurve["coeff"]
    xLim = pyCurve["xLim"]
    if len(coeff) == 1:
        if x <= xLim[0]:
            x = xLim[0]
        elif xLim[1] <= x:
            x = xLim[1]
        p = (coeff[0][0]*x + coeff[0][1])
    else:
        if x <= xLim[0]:
            index = 0
            x = xLim[0]
        elif xLim[0] < x and x <= xLim[1]:
            index = 0
        elif xLim[1] < x and x <= xLim[2]:
            index = 1
        elif xLim[2] < x and x <= xLim[3]:
            index = 2
        elif xLim[3] <= x:
            index = 2
            x = xLim[3]
        p = (coeff[index][0]*x + coeff[index][1])
    return p


def makeMatrix(model, properties):
    numTotalNode = len(model["node"])
    numRealNode = len(model["node"]) - 4

    qVec = np.zeros(numTotalNode)
    coeffVec = np.zeros(numTotalNode)
    forceVec = np.zeros(numTotalNode)
    dispVec = np.zeros(numTotalNode)
    kMatrix = np.zeros([numTotalNode, numTotalNode])
    checkVec = []
    for i in range(numRealNode):
        checkVec.append(False)

    E = properties["pile"]["E"]
    I = properties["pile"]["I"]
    EI = E*I

    for i in range(numTotalNode):
        h = model["node"][i]["dh"]
        kh = model["node"][i]["kh"]

        if i == 0 or i == 1:
            a = [-1, 2, 0, -2, 1] if i == 0 else [0, 1, -2, 1, 0]
            b = np.zeros(numTotalNode-len(a))
            Ki = np.append(a, b)
            q = 0
        elif i == numTotalNode-1 or i == numTotalNode-2:
            a = [-1, 2, 0, -2, 1] if i == numTotalNode-2 else [0, 1, -2, 1, 0]
            b = np.zeros(numTotalNode-len(a))
            Ki = np.append(b, a)
            q = 0
        else:
            a = np.zeros(i-2)
            alpha = 6 + kh * (h**4)/EI
            b = np.array([1, -4, alpha, -4, 1])
            c = np.append(a, b)
            d = np.zeros(numTotalNode - len(a) - len(b))
            Ki = np.append(c, d)

            q = - model["pressure"]["backSide"]["rest"][i] + \
                model["pressure"]["excavationSide"]["rest"][i]

        # qVec[i] = q
        qVec[i] = model["node"][i]["qTest"]
        coeffVec[i] = (h**4)
        forceVec[i] = qVec[i] * coeffVec[i]
        kMatrix[i, :] = Ki * EI

    kMatrix = kMatrix
    forceVec = forceVec.reshape(numTotalNode, 1)
    kInverse = np.linalg.inv(kMatrix)
    dispVec = np.matmul(kInverse, forceVec)

    test = 0
    errMax = 0.0001
    # while False in checkVec and test != 200:
    while False in checkVec and test != 0:
        for i in range(2, numTotalNode-2):
            checkNum = i-2

            pyCurve = model["pyCurve"][i]
            isExcavation = model["node"][i]["isExcavation"]
            dh = model["node"][i]["dh"]
            x = dispVec[i][0]
            xFromCurve = calcDispFromPYCurve(qVec[i] * dh, pyCurve)
            err = xFromCurve - x
            check = err <= abs(errMax)
            checkVec[checkNum] = True if check or isExcavation else False
            # checkVec[checkNum] = True if check else False
            if checkVec[checkNum] == False:
                pNew = calcForceFromPYCurve(x, pyCurve)
                qNew = pNew / dh
                # qVec[i] = qNew
                # qVec[i] = (qNew + qVec[i]) / 2
                # sign = 1 if err<=0 else -1
                # qVec[i] = qNew + sign*model["node"][i]["kh"]*x
                qVec[i] = qVec[i] - 2*kh*dispVec[i][0]
                forceVec[i][0] = qVec[i] * coeffVec[i]

        dispVec = np.matmul(kInverse, forceVec)
        test += 1

    print("test: ", test)
    # print("checkVec: ", checkVec)
    # print("checkVec: ", checkVec)

    return {"qVec": qVec.reshape(1, numTotalNode), "dispVec": dispVec.reshape(1, numTotalNode)}
    # return dispVec.reshape(1, numTotalNode)
